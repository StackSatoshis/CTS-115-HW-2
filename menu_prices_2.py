...
# Asks users to select items from a menu. Then calculates and displays Tax, Tip and Totals
...
def add_items(title, options): # defines the add_items function
    ...
    while True:
        input_key = input("Select a menu item: ") # Assign item key variable
        if input_key is None or len(input_key) == 0:    # Error handling
            break
        try:
            input_key = int(input_key)
            new_items.append(options[input_key])
        except KeyError:                                #
            print("Thats not on the menu, try again")   #
            continue                                    #
        except ValueError:                              #
            print("Thats not on the menu, try again")   # Handles various errors caused by special characters
            continue                                    #
...
def calculate_lunch():
    ...
    lunch_items = add_items("Please select your items. (Press enter to skip to next menu item):",
                             LUNCH_OPTIONS) ## Select your lunch
    sweets_items = add_items("Please select your sweets. (Press enter to skip to next menu item):",
                             SWEETS_OPTIONS) ## Select your Sweets
    coffee_items = add_items("Please select your coffees. (Press enter to skip to next menu item):",
                             COFFEE_OPTIONS) ## Select your Coffees
    tea_items = add_items("Please select your teas. (Press enter to continue):",
                             TEA_OPTIONS) #Select your Teas
